...
I woked with the team in order to write every requirements related to the CLT unit in the project,
...
The requirements are divided in Hith Level Requirements (HLR) and Low Level Requirements (LLR),
each LLR must have one or more HLR paret and each HLR must have one or more LLR child.
...
For each assumptions I alswo wrote about the reasons why we have to consider them revelant for the project.
...
I delated usesless command like @breaf and @par that were misused in the project.
I worked with the Tracability man to ensure a common guidelines for the documentation of the project.
...
\begin{itemize}
    ...
    \item Advisory: A guideline that is only raccomanded (can't just ignore them).
\end{itemize}

For each rule we have a breaf description, a category, the version which the rule applies to, the scope of the analysis (like Single Translation unit), 
...
I inpruved the current logging initializzation system in the main.cc file with a better handling of the log file and when we have to set the 
